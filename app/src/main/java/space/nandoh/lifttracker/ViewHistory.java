...
import space.nandoh.lifttracker.adapters.RecyclerItemClickListener;
...
//        listView.setAdapter(adapter);
        RecyclerView recyclerView = (RecyclerView) findViewById(R.id.listview_history);
        ...
        recyclerView.addOnItemTouchListener(
                new RecyclerItemClickListener(context, recyclerView, new RecyclerItemClickListener.OnItemClickListener() {
                    ...
                    public void onItemClick(View view, int position) {
                        selected_day = days[position].toString();
                        try {
                            SQLiteOpenHelper liftDatabaseHelper = new LiftDatabaseHelper(context);
                            SQLiteDatabase db = liftDatabaseHelper.getReadableDatabase();
                            String tableName = "SAVE_DATA";
                            String[] tableColumns = new String[]{"*"};
                            String query_day = selected_day.replace(" ", "\\");
                            String whereClause = "DATE=" + query_day;
                            String[] whereArgs = null;
                            String groupBy = null;
                            String having = null;
                            String orderBy = null;
                            Cursor cursor_days = db.query(tableName,
                                    ...
                                    orderBy);
                            if (cursor_days.moveToFirst()) {
                                String lift_name = cursor_days.getString(1);
                                String muscle_group = cursor_days.getString(2);
                                String weight_type = cursor_days.getString(3);
                                String reps_weight = cursor_days.getString(4);
                                /*TODO
                                output lift to the view
                                 */
                                while (cursor_days.moveToNext()) {
                                    lift_name = cursor_days.getString(1);
                                    muscle_group = cursor_days.getString(2);
                                    weight_type = cursor_days.getString(3);
                                    reps_weight = cursor_days.getString(4);
                                    /*TODO
                                output lift to the view
                                 */
                                }
