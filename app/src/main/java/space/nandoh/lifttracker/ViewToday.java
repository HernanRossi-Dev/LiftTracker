    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            Cursor cursor = db.query ("SAVE_DATA",
                    new String[] {"DATE", "LIFT_NAME","WEIGHT_TYPE", "REPS_WEIGHT", "TOTAL_WEIGHT"},
                   "DATE = ?",
                   new String[] {date},
                   null, null, null);
            if(cursor.moveToFirst()) {
                String lift_name = cursor.getString(1);
                String weight_type = cursor.getString(2);
                String lift_info = cursor.getString(3);
                String total_weight = cursor.getString(4);
                output = output + lift_name + " done using " + weight_type +  ":\n" +
                        lift_info + "\n" + "The total weight lifted for this set was: " +
                        total_weight + " lbs\n";
                while(cursor.moveToNext()) {
                    lift_name = cursor.getString(1);
                    weight_type = cursor.getString(2);
                    lift_info = cursor.getString(3);
                    total_weight = cursor.getString(4);
                    output = output + " -------------------------------------\n" +
            ...
            Toast toast = Toast.makeText(this, "Failed access the database", Toast.LENGTH_SHORT);
            toast.show();
        }
    }
